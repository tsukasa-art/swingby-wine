# Wukiyo Wine Patch Policy

Last updated: 2026-06-15

This repository is the canonical Wine fork for Wukiyo. The git base is the
...
Wukiyo runtime behavior must not depend on untracked binaries copied from
...
- Reproducible build inputs for artifacts shipped by Wukiyo.
...
- v1 `master` is the minimal clean fork needed to reproduce the current Wukiyo
...
feat(d3d9,wined3d): WUKIYO_CMVS_THUMBS ...
```

This commit is a candidate source explanation for the current Wukiyo bundle,
but the bundle is authoritative until a clean rebuild proves equivalence by
feature probes and hashes.

v1 `master` includes:

- Core patches required by the current Wukiyo bundle.
- `WUKIYO_CMVS_THUMBS` as a default-off engine gate.
...
These patches are part of the supported Wukiyo Wine runtime and should be
...
These patches must be default-off inside Wine and enabled by Wukiyo only for
the engine/title policy that requires them.

- CMVS thumbnail capture and last-presented back-buffer serving.
- Gate: `WUKIYO_CMVS_THUMBS`.
- IPC contract: `/tmp/wukiyo_snap_NNN.bgra` with
  `[u32 width][u32 height][u32 stride] + BGRA pixels`, plus
  `/tmp/wukiyo_page_base.txt`.
...
The shipped Wukiyo runtime must be tied back to source by content, not by
mtime. Before publishing or rebasing:

1. Build from the intended canonical commit.
2. Bundle into Wukiyo.
3. Record the commit, feature probes, and SHA-256 hashes in Wukiyo's
